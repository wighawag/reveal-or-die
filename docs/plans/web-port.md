...
- **The reveal animation, and with it what a reveal actually DID.** Both need one thing: a `CommitmentRevealed` log feed. `lib/world/state.ts` reads standing avatars out of storage and nothing else, so the app never sees the actions the chain accepted, and `ZonesReader` already carries the `fromBlock`/`toBlock` the feed would use (the comment there says as much).

  `bomber-world` is the worked example, and it is the same contract lineage. `web/src/lib/onchain/zones-fetcher.ts` fetches `CommitmentRevealed` for the camera's zones and the last epoch or two alongside the entity read, batching the block range; `web/src/lib/onchain/state.ts` hands each avatar its `previousActions`; `render/objects/AvatarObject.ts` walks a gsap timeline through the moves. Every avatar animates, not only the player's, because the event is per avatar and indexed by zone.

  WHAT THE EVENT CARRIES is the important part: `_reveal` emits `actions[0:numActionsResolved]`, and a refused action sets `stopProcessing` without incrementing, so the slice is EXACTLY the prefix the contract carried out. That is the per-action outcome, and it answers the item below as well as the animation.
...
- A rejected move sets `stopProcessing` and silently drops the remaining actions in the same reveal, and nothing REVERTS to say so. A refused Exit now joins it, deliberately: the alternative is reverting the reveal, which costs the player the whole turn and blocks the next epoch. It is not invisible, though - `CommitmentRevealed` carries the accepted prefix - so what is missing is a client that reads it, not information on chain.
- **What the HUD says a reveal DID is what was revealed, not what the chain accepted.** `describeRound` reads the actions the round last held (`lib/world/reveal-outcome.ts`), because that is all a client without a log feed can see, so a step into a wall is still reported as "your avatar has moved". The feed above closes it: the event says two moves where the player revealed three.
