...
- ~~**The Exit action is unreachable.**~~ **DONE**, `planning.exitAt()`, reachable from the d-pad's "Leave the world" button, the Space or X key, and a gamepad's west button. A planned exit is drawn as a ring round the cell the moves end on, because it has no cell of its own and a dot would be invisible under the last step. **The open question it leaves is a game-design one, recorded below**: the contract does not check where you leave from.
...
- **Leaving is legal from anywhere, and the map says otherwise.** `_exit` ignores its action data entirely, and `UnableToExitFromThisPosition` is declared in `UsingGameErrors.sol` and thrown nowhere, so an avatar may leave from any cell. The client mirrors that (`planning.exitAt`), because the alternative was a rule only this client believed: the pre-port build refused unless `currentCellType == 3`, while any other client could exit from anywhere. But the exit tile IS drawn on the board, and a player will reasonably assume standing on it is required, so the two have to be reconciled in one of two directions. Either `_exit` gains the check the declared error was clearly written for and `planning.exitAt` mirrors it (the tile becomes the goal it looks like), or the tile stops being drawn as a goal (leaving becomes a plain action available anywhere). That is a decision about what the game IS, which is why it was not taken while wiring the controls up. The test named "permits an exit from a cell that is not the exit tile" is the one that has to change with it.
- **An Exit committed for an avatar that is not in the world corrupts the zone index.** `_exit` sets `left` unconditionally, and `_resolveActions` then calls `_removeFromZone(resolution.startZone, avatarID)` for an avatar that is not in that zone's list, which pops whoever IS last in it: another player is quietly evicted from the board. Found while building `exitAt`, which refuses to plan one for exactly this reason, so this client cannot cause it - but the client is not the guard, and any other one can. The fix is in `_exit` or in `_resolveActions`.
