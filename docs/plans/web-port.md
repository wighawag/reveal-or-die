...
- **`test/lib/context/fatal.test.ts` times out in full runs on this repo.** Passing alone (about 40s, most of its 120s budget) and failing under the 16-way parallel run, roughly three runs in four on a loaded machine. Not caused by any one change: the file documents this exact mode, and its budget was set from measurements on the TEMPLATE, whose module graph is smaller than this app's. Every case re-imports the whole context graph. It is byte-identical to stem and says a fork that edits the number "buys itself a conflict on every future alignment", so the fix belongs upstream: either the budget scales with the graph, or the cases stop paying for a fresh import each.
