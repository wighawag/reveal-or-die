...
- **UNREPRODUCED: a fresh browser showed another account's avatar at its previous position and did not update.** Reported once, reloaded before it could be watched. The most likely mechanism is NOT the log feed - a failure there costs the animation, never the board - but the EPOCH CHECK in `readBatch`:

  `if (Number(epoch) !== expectedEpoch) return undefined` is an EXACT match between two independent clocks: the contract's `_epoch()` at whatever block the node served, and this client's `epochInfo`, derived from polled block timestamps. The catchup retry in `lib/onchain/state.ts` is worded for one direction of disagreement ("the node has not reached this epoch yet") and its budget covers that one. But the check also fails when the node is AHEAD of the client - `chainTime` lagging a poll behind, which a cold page is likeliest to be - and in that direction the retry can never succeed: it asks again, gets the same newer epoch, returns undefined again, until the budget expires and the read throws, which is exponential backoff behind the RPC-health banner. The board then FREEZES at whatever the last successful fetch showed - which is exactly the reported symptom: a position from before the reveal, not updating.

  bomber-world's own fetcher checks only one side (`if (Number(fetchedEpoch) < expectedEpoch) throw`), so a node ahead of the client is tolerated there.

  NEXT TIME IT HAPPENS, before reloading: is the RPC-health banner up, and does the board recover if the camera pans? That distinguishes this (banner up, pan does nothing) from a fetch that simply has not run yet. The fix, if confirmed, is one-sided validation in `readBatch` - accepting a node ahead of our clock, which for a board that only DRAWS is the safer direction to be wrong in - but it changes a sync invariant, so it should not be changed blind on the strength of one unwatched occurrence.
