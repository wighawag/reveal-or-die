...
- A rejected move sets `stopProcessing` and silently drops the remaining actions in the same reveal, which is indistinguishable from a reveal that did nothing. A refused Exit now joins it, deliberately: the alternative is reverting the reveal, which costs the player the whole turn and blocks the next epoch. The client refuses to plan either, so the only way to reach it is another client or a bug, but the player is still told nothing.
- **What the HUD says a reveal DID is what was revealed, not what the chain made of it.** `describeRound` reads the actions the round last held (`lib/world/reveal-outcome.ts`), so a move the contract refused is still reported as "your avatar has moved". Closing that needs per-action outcomes, which the contract does not emit at all.
