...
- **What the HUD says a reveal DID is still what was revealed, not what the chain accepted.** `describeRound` reads the actions the round last held (`lib/world/reveal-outcome.ts`), so a step into a wall is reported as "your avatar has moved". The log feed now on the board makes this closable - the player's own avatar carries `lastTurn`, which is the accepted prefix - and it has not been wired up.
