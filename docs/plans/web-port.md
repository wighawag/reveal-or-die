...
- **A purchase should survive a reload.** Today the state lives only in `lib/world/purchase.ts`'s store, so reloading while the transaction is in flight loses it: the setup gate goes back to "Buy an avatar" and the player can pay a second time for an avatar the first transaction is already minting. The subID is random, so the second purchase does not collide with the first, it simply buys another one. BLOCKED on the payment rail's transactions being tracked (see below): once they are recorded as operations, a pending purchase is visible in account data across a reload and the gate can read it, which is the same machinery the signer's moves already use. Do NOT solve it by persisting the subID here; that is a second copy of what the operations ledger exists to hold.
- **Payment-rail transactions are not tracked**, so they never reach the transaction list. `context/core.ts` guards the rail's client with the in-flight ledger but does not wrap it with `trackerBuilder.using`, unlike the account's client and the signer's. The comment above it reasons carefully about the crash window and never mentions the transaction list, so it reads as an oversight rather than a decision, and it is the one client "whose transactions the user paid for on purpose". Reported upstream; it is `with/local-signer`'s to fix, since `main` composes no rail.
