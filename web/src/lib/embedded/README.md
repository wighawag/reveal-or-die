...
3. **A provisioning hook that gives the offline player whatever THIS game puts at stake.** In the reference game that is a bonded ERC20; on `with/nft-identity` and in reveal-or-die it is an identity token they must own; elsewhere it is something else again, because the framework requires only that something is lost by not revealing. `createEmbeddedWorld({provision})` runs after the deploy and before the world exists, and is handed the rocketh environment and the node, so it can call contracts and use `evm_setBalance` for gas. It is a hook rather than a branch edit on purpose: written per branch it would land in four divergence tables forever - which is also why this file, inherited unchanged by every repo below, must not name one game's answer as though it were the framework's.
