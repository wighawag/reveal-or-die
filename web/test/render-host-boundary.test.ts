 ...
 *
 * `$lib/world/render/index.ts` names which canvas host this game mounts,
 ...
 *
 * INHERITED, AND THE ONE CONSTANT BELOW IS THIS GAME'S. Upstream the selector
 * is `$lib/placement/render/index.ts`; this game deleted `lib/placement/`
 * wholesale and keeps its selector at `$lib/world/render/index.ts`. Everything
 * else - the page, the prop extraction, the rule - is the inherited file
 * unchanged, which is the `mode.ts`/`TARGET_STEP` shape: a one-line difference
 * in a file whose job is otherwise identical. The guard is worth keeping here
 * rather than deleting with the rest of `placement`, because this repo mounts
 * the INHERITED pixi host from its own route, so the page-to-host prop contract
 * it protects is a cross-repo one here rather than a cross-branch one.
 */

const root = new URL('..', import.meta.url).pathname;
const PAGE = 'src/routes/play/+page.svelte';
const SELECTOR = 'src/lib/world/render/index.ts';
