...
/**
 * AN EPOCH IS A COMMIT PHASE FOLLOWED BY A REVEAL PHASE AND NOTHING ELSE.
 *
 ...
 * segment, a commitment made in the CURRENT epoch is always still openable: you
 ...
 * case - the moment the reveal window shuts, the epoch has advanced, and the
 * existing missed-reveal path reports it and offers the settlement.
 *
 * A settlement window, a gap between epochs, or an epoch that can turn over
 ...
 *   closing the epoch early is refused until every commitment in it has been
 ...
 *   are commit to reveal within one epoch, and reveal to the next epoch once
...
describe('an epoch has no gap in it', () => {
	it('is in one of exactly two phases at every instant, under the timed formula', () => {
		...
		for (let t = config.startTime; t < config.startTime + 400; t += 0.5) {
			...
			if (info.currentEpoch === previous.currentEpoch) {
				// Within one epoch the only move is commit -> reveal, never back.
				expect(
					previous.isCommitPhase || !info.isCommitPhase,
					`phase went backwards at ${t}`,
				).toBe(true);
			} else {
				// AND AN EPOCH IS ONLY EVER LEFT FROM ITS REVEAL PHASE. Leaving
				// from the commit phase is what would strand a commitment that
				// was made and never given a window to open in.
				expect(info.currentEpoch, `epoch jumped at ${t}`).toBe(
					previous.currentEpoch + 1,
				);
				expect(previous.isCommitPhase, `left epoch early at ${t}`).toBe(false);
	...
	it('tiles time with no hole and no overlap, under the hybrid prediction', () => {
		...
		for (let i = 0; i < 20; i++) {
			...
			expect(next.epoch).toBe(
				reading.isCommitPhase ? reading.epoch : reading.epoch + 1,
			);
			reading = next;
		}
	});

	it('leaves the epoch ending where it did when a reveal window opens early', () => {
