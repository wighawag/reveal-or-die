 ...
 * `scripts/dangling-imports.mjs`; what this adds is that `test:unit` is in this
 * repo's configured verify, so `offshoot-fanout --verify` asks the question too
 * rather than only a human who remembered to.
 *
	...
	it('finds nothing dangling under web/src, web/test and web/e2e', () => {
		...
		expect(
			...
			'these imports name paths this repo does not have. After a cascade ' +
				...
				'arrived importing something this repo deleted (the template plays ' +
				'`$lib/placement`; this game is `$lib/world`). Neither is a conflict, ' +
				'which is why this is checked rather than reviewed',
		).toEqual([]);
	});

	it('has teeth: both shapes of the cascade defect are reported', () => {
		// GUARDS THE GUARD, and this repo has already been broken twice by a check
		...
		// exclusion is exactly the kind of hole this is guarding.
		...
		try {
			...
			writeFileSync(
				...
				"import {x} from '$lib/placement/commit-reveal';\nexport const b = x;\n",
			);
