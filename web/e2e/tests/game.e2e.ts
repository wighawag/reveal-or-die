...
describe('A turn bigger than one transaction', () => {
	...
	test('commits a long turn as a chain and opens it across several reveals', async ({
		...
		//
		// ONE NEW CLAIMANT PER CELL, not one placement cost per cell. Upstream
		// this reads `placementCost` off the deployment and asserts the stake went
		// up by it; here that number is zero by design - custody of the avatar is
		// the stake - so the quantity that moves is who has claimed the cell, and
		// `stakeOnCell` reports that instead. Inheriting upstream's line would
		// have asserted the board did not change, on the one test whose whole
			...
			.poll(
				async () => {
					for (const cellID of cells) {
						...
						if (now !== (before.get(cellID) ?? 0n) + 1n) {
							return false;
						}
					}
					return true;
