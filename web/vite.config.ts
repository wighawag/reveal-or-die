...
export default defineConfig(({mode}) => {
	...
	// The sprite pipeline used to be wired here, in four lines that decided when
	// it should run. It is inherited now (`vite.plugins.ts` -> the template's
	// `vite.assetpack.ts`), and both decisions moved INTO the plugin: it writes
	// the placeholder manifest whenever one is absent, and returns `false` when
	// there is no `../assets` or when running under vitest. So there is nothing
	// left to say here, which is the point of the seam.
