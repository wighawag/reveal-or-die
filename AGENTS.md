...
**The reason is measured, twice, and both times in a descendant.** Two cascades out of this template have broken `reveal-or-die`, and both were broken by a hunk that merged CLEANLY and reported no conflict at all: in 2026-09-09 this template deleted `game/render/pixi/PixiCanvas.svelte` and that repo's own `world/render/index.ts` went on importing it; in 2026-09-23 five files arrived as clean ADDS carrying nine imports of `$lib/placement`, which is THIS repo's game and a directory that one had deleted long ago. Four routine-looking conflicts invite resolve-and-move-on, and `--verify` catches both one step late. **The check lives here rather than only there because the defect is the tree's**: every game deletes the template's game and writes its own, so every one of them is one clean hunk away from the same failure, and a check only the repo that got burnt carries is a check the next repo has to get burnt to acquire. `scripts/dangling-imports.mjs` holds the reasoning and why it is repo-local rather than in `offshoot-fanout`, where it belongs.
