...
## Before you commit a cascade into this repo
...
**The reason is measured, twice.** Both cascades that have broken this repo were broken by a hunk that merged CLEANLY and reported no conflict at all: in 2026-09-09 the template deleted `game/render/pixi/PixiCanvas.svelte` and this repo's own `world/render/index.ts` went on importing it; in 2026-09-23 five files arrived as clean ADDS carrying nine imports of `$lib/placement`, which is the TEMPLATE's game and a directory this repo deleted long ago (this game is `$lib/world`). Four routine-looking conflicts invite resolve-and-move-on, and `--verify` catches both one step late. `scripts/dangling-imports.mjs` holds the reasoning and the reason it is repo-local rather than in `offshoot-fanout`, where it belongs.
